...
interface Item {
  ...
  sellingPrice: number
  quantity: string | null
  quantityTypeId: number // New property to store the quantity type ID
}

const ManualAdditionComponent: React.FC = () => {
  const [quantityOptions, setQuantityOptions] = useState<string[]>([])
  const [responseData, setResponseData] = useState<any>(null) // Store API response data
  const defaultItem = {
    itemName: '',
    costPrice: 0,
    sellingPrice: 0,
    ...
    const fetchQuantities = async () => {
      try {
        ...
        if (response.ok) {
          const responseData = await response.json()
          console.log(responseData)
          // Set the API response data to state
  ...
  const handleItemChange = (index: number, field: keyof Item, value: any) => {
    const newItems = [...items]
    if (field === 'quantity') {
      // Check if responseData is available
      if (responseData) {
        // Check if the selected quantity exists in quantityOptions
        const selectedQuantityIndex = quantityOptions.indexOf(value)
        if (selectedQuantityIndex !== -1) {
          // Extract the quantity type ID from the selected quantity string
          const selectedQuantityType = responseData.data[selectedQuantityIndex]
          newItems[index] = {
            ...newItems[index],
            [field]: value,
            quantityTypeId: selectedQuantityType.id, // Set the quantity type ID
          }
        } else {
          // Handle case where selected quantity doesn't exist in quantityOptions
          console.error("Selected quantity doesn't exist in quantityOptions")
          // You may want to handle this case gracefully, such as setting a default quantity type ID
        }
      }
    } else {
      newItems[index] = { ...newItems[index], [field]: value }
  ...
  const handleAddInventory = async () => {
    const itemsToSend = items.map((item) => ({
      ...
      cost_price: item.costPrice,
      sale_price: item.sellingPrice,
      quantity_type_id: item.quantityTypeId, // Use the quantity type ID
    }))
    try {
      for (const item of itemsToSend) {
        console.log('Sending item:', item)
  ...
  const handleAddQuantityModalSubmit = () => {
    ...
    if (newQuantity && newUnit) {
      ...
      fetch('http://localhost:8000/api/quantity-type', {
        ...
        .then((data) => {
          console.log('Success:', data)
          // Log quantityOptions array before and after updating
